...
Vite + React SPA + **Tailwind CSS v4** (플러그인 방식, `tailwind.config.js` 없음 — `@tailwindcss/vite`로 직접 통합). `src/api/client.js`는 axios 기반, `localStorage`의 Bearer 토큰으로 인증, 401 시 자동 로그아웃.

- **인증 상태**: `src/context/AuthContext.jsx`
- **테마**: `src/context/ThemeContext.jsx`
- **라우팅**: `src/App.jsx` (react-router-dom) — `src/pages/`에 페이지, `src/components/`에 공통 컴포넌트
- **개발 프록시**: `vite.config.js`에서 `/api`, `/static` → `http://localhost:3030`

테스트 스위트 없음 (Python/JS 모두). 기능 확인은 실제 앱 실행으로.
